...
# 使用git bash : ls > usdcnh_list, 然后读取文件中的内容,去拿文件;
import os
import math
import numpy as np
import pandas as pd
...
def process_all(csvs):  # n 代表一次读几个文件, n应该小于len(csvs)
    total = len(csvs)
    count = 0
    results = []
    ...
    result_df.to_csv(f"数据质量报告{total}-最后.csv",index=False)
    return result_df
...
def process(df,count,total): 
    ...
    for i in dates[:end_index]: 
        ...
        f = lambda t : int(t[:2])*3600 + int(t[3:5])*60 + int(t[6:8]) + int(t[9:])/1000
...
if __name__=="__main__":
    path = r'C:\Users\chenchao\Desktop\数据质量检查\USDCNY&USDCNH\USDCNH'
    csvs = read_list(path)
    csvs = csvs[998:]
    df = process_all(csvs)  # 为了测试, 返回了df, 实际并不需要  
